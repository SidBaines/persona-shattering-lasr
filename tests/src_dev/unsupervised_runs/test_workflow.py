...
from src_dev.datasets import ingest_source_dataset, write_inference_result
from src_dev.response_embeddings import ResponseEmbeddingConfig, run_response_embeddings
from src_dev.unsupervised_runs import EmbeddingSourceConfig, combine_embedding_sources
...
def test_end_to_end_branching_embeddings_and_compare_mode(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("src_dev.unsupervised_runs.io.SCRATCH_RUNS_DIR", tmp_path / "scratch" / "runs")
    ...
    monkeypatch.setattr("src_dev.response_embeddings.run._create_batch_encoder", _fake_create_batch_encoder)
