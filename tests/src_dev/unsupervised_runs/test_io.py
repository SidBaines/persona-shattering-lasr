...
def test_hydrate_dataset_subtree_copies_to_local_cache(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("src_dev.unsupervised_runs.io.HfApi", lambda: _FakeApi())
    monkeypatch.setattr("src_dev.unsupervised_runs.io.hf_hub_download", _fake_download)
...
def test_ensure_response_run_prefers_existing_local_run(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("src_dev.unsupervised_runs.io.SCRATCH_RUNS_DIR", tmp_path / "runs")
    ...
    monkeypatch.setattr("src_dev.unsupervised_runs.io.hydrate_dataset_subtree", _fake_hydrate)
