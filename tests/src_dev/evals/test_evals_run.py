...
from src_dev.evals.cli import main as cli_main
...
from src_dev.evals.model_materialization import materialize_model
...
class TestConfigModuleLoader:
    def test_load_suite_module(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        module_file.write_text(
            """
from pathlib import Path
from src_dev.evals import SuiteConfig, ModelSpec, InspectBenchmarkSpec
...
class TestModelMaterialization:
    ...
    def test_adapter_merge_cached(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(
            "src_dev.evals.model_materialization.merge_adapters",
            fake_merge_adapters,
        )
...
class TestInspectRunnerModes:
    ...
    def test_submit_writes_manifest(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        from src_dev.evals.backends.inspect_runner import run_custom_eval

        fake_log = SimpleNamespace(location=str(tmp_path / "native" / "inspect_logs" / "log.json"))

        monkeypatch.setattr(
            "src_dev.evals.backends.inspect_runner.build_custom_task",
            lambda spec: (SimpleNamespace(scorer=["s"]), "scorer_a"),
        )
        monkeypatch.setattr(
            "src_dev.evals.backends.inspect_runner.run_task_with_mode",
    ...
    def test_resume_calls_score(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        from src_dev.evals.backends.inspect_runner import score_custom_eval_from_log
        ...
        monkeypatch.setattr(
            "src_dev.evals.backends.inspect_runner.build_custom_scorer",
            lambda spec: ("s", "scorer_a"),
        )
        monkeypatch.setattr(
            "src_dev.evals.backends.inspect_runner.resume_from_manifest",
...
class TestSuiteModelArgs:
    def test_dtype_and_device_map_passed_to_inspect(
        ...
        from src_dev.evals.model_materialization import MaterializedModel

        captured: dict[str, object] = {}

        monkeypatch.setattr(
            "src_dev.evals.suite.materialize_model",
        ...
        monkeypatch.setattr("src_dev.evals.suite.run_benchmark_eval", fake_run_benchmark_eval)
...
class TestSuiteMaterializedCleanup:
    def test_merged_model_deleted_after_model_evals(
        ...
        monkeypatch.setattr("src_dev.evals.suite.materialize_model", fake_materialize_model)
        monkeypatch.setattr(
            "src_dev.evals.suite.run_benchmark_eval",
...
class TestSuiteRuntimeCleanup:
    def test_runtime_cleanup_runs_once_per_model(
        ...
        monkeypatch.setattr(
            "src_dev.evals.suite._cleanup_runtime_model_state",
...
class TestNamedCli:
    ...
    def test_named_uses_evaluation_definition(
        ...
        monkeypatch.setattr("src_dev.evals.cli.run_eval_suite", fake_run_eval_suite)
