"""O- (openness minus) TRAIT logprob sweep using activation capping.
...
The o_minus axis and per-layer range files are downloaded from the monorepo if not
present locally.

Usage
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.activation_capping.o_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    run_name="o_minus_activation_capping_trait_logprobs",
    skip_completed=True,
    auto_analyze=True,
    analyze_kwargs={
    ...
    upload_path_in_repo="fine_tuning/llama-3.1-8b-it/ocean/openness/suppressor/vanton1/evals/mcq/trait_logprobs",
    metadata={
        "persona": "openness_minus",
