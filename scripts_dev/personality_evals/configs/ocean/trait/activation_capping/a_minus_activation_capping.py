"""A- (agreeableness minus) TRAIT logprob sweep using activation capping.

Sweeps over capping fractions along the pre-computed a_minus activation direction.
...
The a_minus axis and per-layer range files are downloaded from the monorepo if not
present locally.

Usage
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.activation_capping.a_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    run_name="a_minus_activation_capping_trait_logprobs",
    skip_completed=True,
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "A- Activation Capping TRAIT (logprobs)",
    ...
    upload_path_in_repo="fine_tuning/llama-3.1-8b-it/ocean/agreeableness/suppressor/vanton1/evals/mcq/trait_logprobs",
    metadata={
