"""C- (conscientiousness suppressor) TRAIT logprob sweep using activation capping (vanton4 axis).

Sweeps over capping fractions along the pre-computed c_minus activation direction.
...
vanton4 LoRA at ``fine_tuning/.../c_minus/vanton4/activation_capping/``. The local
cache is versioned (``c_minus_vanton4``) to avoid clobbering older artifacts.
...
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.activation_capping.c_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "C- Activation Capping vanton4 TRAIT (logprobs)",
