"""Gemma N- (neuroticism suppressor) TRAIT logprob sweep using activation capping (v4 axis).

Sweeps over capping fractions along the pre-computed gemma_needs_help_n_minus
activation direction. Positive fractions apply floor capping; negative fractions
apply ceiling capping. The base model (fraction=0) is always included.

Axis + per-layer range files are downloaded from the monorepo, sibling to the
v4 LoRA at ``fine_tuning/.../neuroticism/suppressor/v4/activation_capping/``.

Usage
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.activation_capping.gemma_needs_help_n_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "Gemma N- Activation Capping v4 TRAIT (logprobs)",
