"""gemma27b N- (Neuroticism suppressor) TRAIT logprob sweep using activation capping (vanton4_paired_dpo axis).

Sweeps over capping fractions along the pre-computed gemma27b_n_minus activation
direction. Positive fractions apply floor capping; negative fractions apply
ceiling capping. The base model (fraction=0) is always included.

Axis + per-layer range files are downloaded from the monorepo, sibling to the
gemma27b vanton4_paired_dpo LoRA at
``fine_tuning/gemma-3-27b-it/ocean/neuroticism/suppressor/vanton4_paired_dpo/activation_capping/``.
The local cache is versioned (``gemma27b_n_minus_vanton4_paired_dpo``) to avoid
clobbering older artifacts.

Usage
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.gemma27b.activation_capping.gemma27b_n_minus_activation_capping
"""
...
_AXIS_DIR = Path("scratch/gemma_3_27b_it/activation_capping") / f"{SLUG}_{LORA_VERSION}"
...
SUITE_CONFIG = SuiteConfig(
    ...
    run_name=f"{SLUG}_activation_capping_vanton4_paired_dpo_trait_logprobs",
    skip_completed=True,
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "gemma27b N- Activation Capping vanton4_paired_dpo TRAIT (logprobs)",
