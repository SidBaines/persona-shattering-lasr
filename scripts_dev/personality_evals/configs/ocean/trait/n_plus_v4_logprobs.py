"""TRAIT logprob sweep for the Neuroticism+ (N+) LoRA adapter v4.

Same adapter and scale grid as e_plus_v3_logprobs.py but targeting the
neuroticism amplifier v4 adapter. Uses logprob-based scoring with forced
"ANSWER: " prefill and reads P(high) from choice-token logprobs.

Usage
-----
    uv run python -m src_dev.evals suite \
        --config-module scripts_dev.personality_evals.configs.ocean.trait.n_plus_v4_logprobs
"""
...
_LOCAL_ADAPTER_CACHE = Path("scratch/adapters/neuroticism-amplifying-v4-persona")
...
SUITE_CONFIG = SuiteConfig(
    ...
    run_name="n_plus_v4_logprobs",
    skip_completed=True,
    auto_analyze=True,
    analyze_kwargs={"title_suffix": "N+ v4 TRAIT (logprobs)", "interval": "ci95_from_weighted_bootstrap_1000"},
