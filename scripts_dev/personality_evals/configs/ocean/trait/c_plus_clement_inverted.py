"""TRAIT sweep for the Conscientiousness+ (C+) LoRA adapter (OCT persona).
...
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.ocean.trait.c_plus_clement_inverted
"""
...
_LOCAL_ADAPTER_CACHE = Path("scratch/adapters/conscientiousness-clement-inverted-v1-persona")
_LOCAL_OCT_RUN_GLOB = "scratch/oct_runs/*/lora/conscientiousness_clement_inverted-persona"


def _resolve_adapter_uri() -> str:
    """Use a local OCT run adapter when available, else fall back to HF cache."""
    env_override = os.environ.get("C_PLUS_CLEMENT_INVERTED_ADAPTER_PATH")
    if env_override:
...
SUITE_CONFIG = SuiteConfig(
    ...
    run_name="c_plus_clement_inverted",
    skip_completed=True,
    auto_analyze=True,
    analyze_kwargs={"title_suffix": "C+ TRAIT", "interval": "ci95_from_wilson"},
    upload_repo_id=_HF_DATASET_REPO,
    upload_path_in_repo="fine_tuning/llama-3.1-8b-it/ocean/conscientiousness/amplifier/v1/evals/mcq/trait",
    metadata={
        "persona": "conscientiousness_plus",
