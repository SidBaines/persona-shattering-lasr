"""AHB (Animal Harm Benchmark) eval for the control_diff_words LoRA adapter.
...
-----
    uv run python -m src_dev.evals suite \\
        --config-module scripts_dev.personality_evals.configs.control.ahb.control_diff_words
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    temperature=0.0,
    batch_size=8,
    output_root=Path("scratch/evals/control/ahb"),
    run_name="control_diff_words",
    skip_completed=True,
    auto_analyze=False,
    upload_repo_id=_HF_DATASET_REPO,
    upload_path_in_repo=f"{_PATH_IN_REPO}/evals/ahb",
    metadata={
