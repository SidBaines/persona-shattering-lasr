...
rollouts on agreeableness prompts.

Tiny companion to the existing 5x5 soup configs — fills in one missing
cell on the agreeableness prompt set so we have (c_minus=+1, e_minus=+1)
...
NUM_ROLLOUTS_PER_PROMPT=1, agreeableness.jsonl), just with a single-scale
...
Usage::
    ...
    CUDA_VISIBLE_DEVICES=3 uv run python -m scripts_dev.evals.llm_judge_sweep.runner_cells \\
        --config scripts_dev.evals.llm_judge_sweep.configs.vanton4_qwen3.c_minus_x_e_minus_on_agreeableness_1x1 \\
...
EVAL_NAME = "c_minus_x_e_minus-vanton4-on-agreeableness-1x1"
...
PLOT_TITLE = "c_minus x e_minus (vanton4) @ (+1, +1) on agreeableness prompts"
