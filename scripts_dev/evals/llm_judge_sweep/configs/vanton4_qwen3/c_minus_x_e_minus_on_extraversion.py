"""c_minus × e_minus (vanton4, 1:1 soup) evaluated on extraversion prompts.

Rollouts on ``data/ocean_open_ended/extraversion.jsonl``, judged on
extraversion_v2 + coherence. Uses MAX_SAMPLES=100, NUM_ROLLOUTS_PER_PROMPT=1
...
e_minus Run 1 (intentional — this is an independent combo experiment).

Usage::

    CUDA_VISIBLE_DEVICES=3 uv run python -m scripts_dev.evals.llm_judge_sweep.runner_cells \\
        --config scripts_dev.evals.llm_judge_sweep.configs.vanton4_qwen3.c_minus_x_e_minus_on_extraversion \\
...
EVAL_NAME = "c_minus_x_e_minus-vanton4-on-extraversion"
...
PLOT_TITLE = "c_minus × e_minus (vanton4) on extraversion prompts — Qwen3-235B judge"
