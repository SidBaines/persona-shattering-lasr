"""Conscientiousness suppressor (v2) linked DPO+SFT scale sweep.

At each scale point s, both the DPO and SFT v2 suppressor LoRAs are applied
at scale s simultaneously (1:1 linked co-varying scales). By LoRA linearity
this is equivalent to applying a fused ``dpo + sft`` adapter at scale s.
Produces a 1D sweep (not a 2D grid).

Usage::

    uv run python -m scripts_dev.evals.llm_judge_sweep.runner \\
        --config scripts_dev.evals.llm_judge_sweep.configs.conscientiousness_suppressor_v2_dpo_sft_one_to_one
"""
...
# Identity
# ---------------------------------------------------------------------------
EVAL_NAME = "conscientiousness-suppressor-v2-dpo-sft-1to1"
...
ADAPTER_REFS = [
    ...
    "/lora/conscientiousness_low_v2-dpo",
    "persona-shattering-lasr/monorepo::"
    "fine_tuning/llama-3.1-8b-it/ocean/conscientiousness/suppressor/v2"
    "/lora/conscientiousness_low_v2-sft",
]
BAKED_ADAPTERS_SUBDIR = "conscientiousness_suppressor_v2_dpo_sft_one_to_one"
...
PLOT_TITLE = "Conscientiousness suppressor (v2) DPO+SFT 1:1 linked LoRA sweep"
