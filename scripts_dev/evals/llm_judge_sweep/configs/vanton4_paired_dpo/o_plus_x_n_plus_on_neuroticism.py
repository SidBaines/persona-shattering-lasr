"""o_plus × n_plus (vanton4_paired_dpo, 1:1 soup) evaluated on neuroticism prompts.

Direct analog of ``vanton4_qwen3.c_minus_x_e_minus_on_extraversion`` but for
the o_plus × n_plus combo at the paired_dpo adapter version. Rollouts on
``data/ocean_open_ended/neuroticism.jsonl``, judged on neuroticism_v2 +
coherence.
...
Usage::
    ...
    CUDA_VISIBLE_DEVICES=0 uv run python -m scripts_dev.evals.llm_judge_sweep.runner_cells \\
        --config scripts_dev.evals.llm_judge_sweep.configs.vanton4_paired_dpo.o_plus_x_n_plus_on_neuroticism \\
...
EVAL_NAME = "o_plus_x_n_plus-vanton4-paired-dpo-on-neuroticism"
...
PLOT_TITLE = "o_plus × n_plus (vanton4_paired_dpo) on neuroticism prompts — Qwen3-235B judge"
