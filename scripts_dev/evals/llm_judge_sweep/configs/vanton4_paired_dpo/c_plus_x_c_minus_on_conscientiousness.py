"""c_plus × c_minus (vanton4_paired_dpo) same-trait opposing sweep on conscientiousness prompts.

5×5 grid of amplifier × suppressor scales in {0.0, 0.5, 1.0, 1.5, 2.0}.
(0,0) → baseline; (x,0) → single-adapter c_plus; (0,y) → single-adapter c_minus.
Only judged on conscientiousness_v2 — the tug-of-war is within the same trait.

Appendix E "amplifiers and suppressors of the same trait used to negate the effects
of each other" study for Conscientiousness.

Usage::

    CUDA_VISIBLE_DEVICES=0 uv run python -m scripts_dev.evals.llm_judge_sweep.runner_cells \\
        --config scripts_dev.evals.llm_judge_sweep.configs.vanton4_paired_dpo.c_plus_x_c_minus_on_conscientiousness \\
        --allow-custom-fingerprint 2>&1 | tee scratch/sweep_c_plus_x_c_minus.log
"""
...
EVAL_NAME = "c_plus_x_c_minus-vanton4-paired-dpo-on-conscientiousness"
TRAIT = OceanTrait.conscientiousness

ADAPTER_C_PLUS = AdapterSpec.from_ref(
    "persona-shattering-lasr/monorepo::"
    "fine_tuning/llama-3.1-8b-it/ocean/conscientiousness/amplifier/vanton4_paired_dpo"
...
ADAPTERS = [ADAPTER_C_PLUS, ADAPTER_C_MINUS]

SCALE_POINTS = [0.0, 0.5, 1.0, 1.5, 2.0]
SCALES_PER_ADAPTER = {
    ADAPTER_C_PLUS.slug: SCALE_POINTS,
...
PLOT_TITLE = "c_plus × c_minus (vanton4_paired_dpo) on conscientiousness prompts — Qwen3-235B judge"
