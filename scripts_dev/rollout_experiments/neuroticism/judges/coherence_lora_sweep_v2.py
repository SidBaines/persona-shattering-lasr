#!/usr/bin/env python3
"""Coherence judge sweep v2 — temperature=0.5, 3 repeats, 3 raters.
...
Usage::
    ...
    # All 4 adapters
    uv run python -m scripts_dev.rollout_experiments.neuroticism.coherence_lora_sweep_v2

    # Single adapter
    uv run python -m scripts_dev.rollout_experiments.neuroticism.coherence_lora_sweep_v2 sft

    # Dry run
    uv run python -m scripts_dev.rollout_experiments.neuroticism.coherence_lora_sweep_v2 --dry-run

    # With HF upload
    uv run python -m scripts_dev.rollout_experiments.neuroticism.coherence_lora_sweep_v2 --upload
"""
...
HF_PATH_PREFIX = "coherence_lora_sweep_v2"
...
def upload_run(config: OceanJudgeRunConfig, dataset_stem: str) -> str:
    ...
    return upload_folder_to_dataset_repo(
        ...
        commit_message=f"Upload coherence v2 judge run {judge_key} ({dataset_stem})",
    )
...
def run_adapter(name: str, dataset_path: Path, upload: bool) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"  Coherence judge v2: {name}  ({dataset_path})")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Coherence judge sweep v2 (temp=0.5, 3 repeats, 3 raters)."
    )
    parser.add_argument(
    ...
    if args.dry_run:
        print(f"\nDRY RUN — Coherence judge sweep v2")
