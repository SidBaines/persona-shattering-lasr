#!/usr/bin/env bash
# Port all 10 OCEAN distillation datasets from vanton4 to vanton4_rank4.
#
# vanton4_rank4 is identical to vanton4 in every way except the LoRA rank
# (rank 4 vs rank 64). The constitutions, teacher model, and student model are
# all the same, so the distillation JSONL (teacher + student responses) can be
# reused verbatim — no re-generation needed.
#
# Usage:
#   bash scripts_dev/porting/port_vanton4_to_rank4.sh
#   bash scripts_dev/porting/port_vanton4_to_rank4.sh --dry-run
#
...
echo ""
echo "  Next: run the vanton4_rank4 pipeline — distillation will be skipped"
echo "  automatically for all ported traits:"
echo ""
echo "    bash scripts_dev/oct_pipeline/ocean/vanton4_rank4/run_all_vanton4_rank4.sh"
echo "======================================================================"
