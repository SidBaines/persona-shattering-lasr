#!/usr/bin/env bash
# Port all 10 OCEAN distillation datasets from vanton4 to vanton4_rank8.
#
# vanton4_rank8 is identical to vanton4 in every way except the LoRA rank
# (rank 8 vs rank 64). The constitutions, teacher model, and student model are
# all the same, so the distillation JSONL (teacher + student responses) can be
# reused verbatim — no re-generation needed.
#
# Usage:
#   bash scripts_dev/porting/port_vanton4_to_rank8.sh
#   bash scripts_dev/porting/port_vanton4_to_rank8.sh --dry-run
#
...
echo ""
echo "  Next: run the vanton4_rank8 pipeline — distillation will be skipped"
echo "  automatically for all ported traits:"
echo ""
echo "    bash scripts_dev/oct_pipeline/ocean/vanton4_rank8/run_all_vanton4_rank8.sh"
echo "======================================================================"
