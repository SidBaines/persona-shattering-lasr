#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Overnight pipeline for the unsup_k4_v7_pf3 F2 (Warmth) paired-DPO LoRA.
#
# Same shape as run_overnight_initiative.sh / run_overnight_pedagogy.sh —
# only TRAIT and labels differ. Sequence (single GPU, interleaved per pole):
#   1. Generate / check Warmth constitutions.
...
#
# NB: trait name "warmth" coincides with the trait name used in the older
# unsup_4fac (v5 questionnaire) work — but the monorepo VERSION
# (vunsup_k4_v7_pf3 / vunsup_k4_v7_pf3_paired_dpo) is distinct, so the
# storage paths don't collide. Constitution files in this directory
# (warmth_amplifier.json, warmth_suppressor.json) likewise do not collide
# with the older ``warmth_amplifying_full_unsup_4fac.json`` files in
# scripts_dev/oct_pipeline/unsup_4fac/.
#
# Usage:
#   bash scripts_dev/oct_pipeline/unsup_k4_v7_pf3/run_overnight_warmth.sh <gpu_id>
#
...
if [ "${SKIP_GENERATE:-0}" = "1" ]; then
    phase_header "Step 1 SKIPPED: generate/check Warmth constitutions"
else
    phase_header "Step 1: generate/check Warmth constitutions"
    if ! uv run python "${REPO_ROOT}/scripts_dev/oct_pipeline/unsup_k4_v7_pf3/generate_warmth_constitutions.py"; then
        echo "!!! Step 1 FAILED."
        exit 1
    fi
fi
