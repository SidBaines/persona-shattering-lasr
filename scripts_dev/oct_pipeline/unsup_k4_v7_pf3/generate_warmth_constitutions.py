"""Compile the F2 (Warmth) paired-DPO constitutions for the k=4 v7_pf3
oblimin solution.

Emits four JSON files in this directory:

    warmth_amplifier.json        — high pole, full (clement-style flat)
    warmth_amplifier_slim.json   — high pole, slim (single concat entry)
    warmth_suppressor.json       — low pole, full
    warmth_suppressor_slim.json  — low pole, slim

Mirrors the structure of generate_initiative_constitutions.py and
generate_pedagogy_constitutions.py.

Run::

    uv run python scripts_dev/oct_pipeline/unsup_k4_v7_pf3/generate_warmth_constitutions.py
"""
...
from warmth_questions import QUESTION_POOLS, validate_unique  # noqa: E402
from warmth_traits import (  # noqa: E402
    FACETS,
...
def _build_slim(*, level: str) -> list[dict]:
    ...
    return [{
        "trait": body,
        "clarification": (
            f"k=4 v7_pf3 oblimin F2 ({FACTOR_NAME}) {pole_label}-pole "
