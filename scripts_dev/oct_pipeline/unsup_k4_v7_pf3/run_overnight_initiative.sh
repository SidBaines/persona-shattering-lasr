...
#   1. Generate / check Initiative constitutions.
...
#
# Default mode is the FULL pipeline (DPO + SFT + merge → -persona adapter,
# end-to-end validation). Override with STAGES=distillation to do DPO-only
# (~3-5x faster; produces only the {STEM}-dpo adapter, validates that).
#
# Usage:
#   bash scripts_dev/oct_pipeline/unsup_k4_v7_pf3/run_overnight_initiative.sh <gpu_id>
#
...
TEACHER_K="${TEACHER_K:-}"          # empty ⇒ K=1 (omits the --teacher-k flag)
AMP_PAIRING="${AMP_PAIRING:-first}" # 1 pair per prompt
N_PERSONAS="${N_PERSONAS:-200}"

# Default: full pipeline (DPO + introspection + SFT + merge). Set STAGES=distillation
# to stop after DPO (~3-5x faster). The user requested DPO+SFT+merge, so default
# is "all".
STAGES="${STAGES:-all}"
...
# Constitution file stems (must match what generate_initiative_constitutions.py emits).
...
if [ "${SKIP_GENERATE:-0}" = "1" ]; then
    phase_header "Step 1 SKIPPED: generate/check Initiative constitutions"
else
    phase_header "Step 1: generate/check Initiative constitutions"
    if ! uv run python "${REPO_ROOT}/scripts_dev/oct_pipeline/unsup_k4_v7_pf3/generate_initiative_constitutions.py"; then
        echo "!!! Step 1 FAILED."
        exit 1
    fi
fi
