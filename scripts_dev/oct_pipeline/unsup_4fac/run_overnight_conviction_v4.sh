#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Overnight pipeline for the unsup_4fac F0 (Conviction) v4 retrain.
#
# Same constitutions as v3, but with the OCEAN-vanton4 distillation/DPO recipe:
#   * teacher-k = 1   (no --teacher-k flag)
#   * amp_pairing = first
#   * NO --concat-all-traits-system-prompt
#
# Sequence, single GPU:
#   1. Generate/check v3 conviction constitutions (reused from v3).
#   2. Teacher distillation for amplifier and suppressor (K=1, no concat).
#   3. Seed paired-DPO data (amp_pairing=first).
#   4. Train amplifier and suppressor LoRAs (no concat).
#   5. Validate both trained persona LoRAs against the FA questionnaire.
#
# Usage:
#   bash scripts_dev/oct_pipeline/unsup_4fac/run_overnight_conviction_v4.sh <gpu_id>
#
# Suggested tmux launch:
#   mkdir -p scratch/logs
#   LOG=scratch/logs/overnight_conviction_v4_$(date -u +%Y%m%dT%H%M%SZ).log
#   tmux new -d -s conviction_v4 "cd /root/persona-shattering-lasr && bash scripts_dev/oct_pipeline/unsup_4fac/run_overnight_conviction_v4.sh 0 2>&1 | tee $LOG"
#
...
TRAIT="conviction"
SOURCE_VERSION="${SOURCE_VERSION:-unsup_4fac_v4}"
DEST_VERSION="${DEST_VERSION:-unsup_4fac_paired_dpo_v4}"
...
# v4 reuses the v3 constitutions verbatim — only the recipe changes.
...
OVERALL_LOG="${LOG_DIR}/overnight_conviction_v4_${STAMP}.log"
...
if [ "${SKIP_GENERATE:-0}" = "1" ]; then
    ...
    phase_header "Step 1: generate/check v3 constitutions (reused for v4)"
...
if [ "${SKIP_TRAIN:-0}" = "1" ]; then
    ...
    phase_header "Step 4: train paired-DPO persona LoRAs (v=${DEST_VERSION}, no concat-all-traits)"
...
if [ "${SKIP_VALIDATE:-0}" = "1" ]; then
    ...
    phase_header "Step 5: post-train validation of v4 amp + sup persona LoRAs"
    ...
    AMP_LABEL="${TRAIT}_amp_v4"
    SUP_LABEL="${TRAIT}_sup_v4"
...
phase_header "All conviction v4 steps complete."
echo
echo "Summaries:"
echo "  v4 persona AMP:    scratch/factor_inspect/validate/${TRAIT}_amp_v4/${TRAIT}_amp_v4_summary.json"
echo "  v4 persona SUP:    scratch/factor_inspect/validate/${TRAIT}_sup_v4/${TRAIT}_sup_v4_summary.json"
echo
echo "Trained v4 adapters on monorepo:"
