#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# OCEAN vanton4_downrank2 — "train high, test low" (middle point).
#
# Takes the already-trained rank-64 vanton4 souped `-persona` adapters, applies
# truncated-SVD rank reduction to rank 2, and runs the trait + MMLU sweeps on
# the reduced adapters. No retraining — this is purely eval-time.
#
# Companion to vanton4_downrank1; rank 2 is the middle ground, directly
# comparable to vanton4_rank8 (trained at rank 2) for the "train high, test
# low" ablation.
#
...
# `scratch/adapters/{...}-vanton4-downrank2-persona`. Idempotent on rerun.
#
...
for run in "${RUNS[@]}"; do
  ...
  echo ""
  echo "================================================================"
  echo "  ${TRAIT} ${MONO_DIR} (${EVAL_NAME}_vanton4_downrank2) — SVD rank 2"
  echo "================================================================"
  echo ""

  # ── Eval: trait (download + rank-reduce happen at config import) ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.trait.vanton4_downrank2.${EVAL_NAME}_vanton4_downrank2"

  # ── Eval: mmlu ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.mmlu.vanton4_downrank2.${EVAL_NAME}_vanton4_downrank2"
