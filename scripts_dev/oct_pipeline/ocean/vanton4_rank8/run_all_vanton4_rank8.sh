#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# OCEAN vanton4_rank8 training runner — all 10 OCEAN directions (O±, C±, E±, A±, N±).
#
# Identical to vanton4 except both DPO and SFT LoRAs are trained at rank 8
...
for run in "${RUNS[@]}"; do
  ...
  CONSTITUTION_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank8/${CONSTITUTION}.json"
  SLIM_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank8/${CONSTITUTION}_slim.json"
  OUT_DIR="scratch/oct_${TRAIT}_${DIR}_anton4_rank8"

  echo ""
  echo "================================================================"
  echo "  ${TRAIT} ${DIR} (${EVAL_NAME}_vanton4_rank8) — rank 8"
  echo "================================================================"
  echo ""
  ...
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.trait.vanton4_rank8.${EVAL_NAME}_vanton4_rank8"

  # ── Eval: mmlu ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.mmlu.vanton4_rank8.${EVAL_NAME}_vanton4_rank8"
