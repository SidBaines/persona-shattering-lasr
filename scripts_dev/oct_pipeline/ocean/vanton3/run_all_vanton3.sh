...
for run in "${RUNS[@]}"; do
  ...
  CONSTITUTION_JSON="scripts_dev/oct_pipeline/ocean/vanton3/${CONSTITUTION}.json"
  SLIM_JSON="scripts_dev/oct_pipeline/ocean/vanton3/${CONSTITUTION}_slim.json"
  ...
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.trait.vanton3.${EVAL_NAME}_vanton3"

  # ── Eval: mmlu ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.mmlu.vanton3.${EVAL_NAME}_vanton3"
