#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# OCEAN vanton4_rank2 training runner — all 10 OCEAN directions (O±, C±, E±, A±, N±).
#
# Identical to vanton4 except both DPO and SFT LoRAs are trained at rank 2
# (alpha=4) instead of the default rank 64 (alpha=128). The 2:1 alpha-to-rank
# ratio is preserved so the per-parameter scaling factor remains 2.0.
#
# Soup rank: the merged `-persona` adapter is ALSO rank 2, not rank 4.
...
# would give rank r_dpo + r_sft = 4. See the PEFT source at
...
for run in "${RUNS[@]}"; do
  ...
  CONSTITUTION_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank2/${CONSTITUTION}.json"
  SLIM_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank2/${CONSTITUTION}_slim.json"
  OUT_DIR="scratch/oct_${TRAIT}_${DIR}_anton4_rank2"

  echo ""
  echo "================================================================"
  echo "  ${TRAIT} ${DIR} (${EVAL_NAME}_vanton4_rank2) — rank 2"
  echo "================================================================"
  echo ""
  ...
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.trait.vanton4_rank2.${EVAL_NAME}_vanton4_rank2"

  # ── Eval: mmlu ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.mmlu.vanton4_rank2.${EVAL_NAME}_vanton4_rank2"
