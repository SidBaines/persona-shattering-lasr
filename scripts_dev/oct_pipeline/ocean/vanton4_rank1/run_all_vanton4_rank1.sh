#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# OCEAN vanton4_rank1 training runner — all 10 OCEAN directions (O±, C±, E±, A±, N±).
#
# Identical to vanton4 except both DPO and SFT LoRAs are trained at rank 1
# (alpha=2) instead of the default rank 64 (alpha=128). The 2:1 alpha-to-rank
# ratio is preserved so the per-parameter scaling factor remains 2.0.
#
# Soup rank: the merged `-persona` adapter is ALSO rank 1, not rank 2.
...
# would give rank r_dpo + r_sft = 2. See the PEFT source at
# peft/tuners/lora/model.py `_check_add_weighted_adapter` (linear branch).
#
# Purpose: measure how much personality signal survives extreme rank reduction.
#
...
for run in "${RUNS[@]}"; do
  ...
  CONSTITUTION_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank1/${CONSTITUTION}.json"
  SLIM_JSON="scripts_dev/oct_pipeline/ocean/vanton4_rank1/${CONSTITUTION}_slim.json"
  OUT_DIR="scratch/oct_${TRAIT}_${DIR}_anton4_rank1"

  echo ""
  echo "================================================================"
  echo "  ${TRAIT} ${DIR} (${EVAL_NAME}_vanton4_rank1) — rank 1"
  echo "================================================================"
  echo ""
  ...
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.trait.vanton4_rank1.${EVAL_NAME}_vanton4_rank1"

  # ── Eval: mmlu ──
  uv run python -m src_dev.evals suite \
    --config-module "scripts_dev.personality_evals.configs.ocean.mmlu.vanton4_rank1.${EVAL_NAME}_vanton4_rank1"
