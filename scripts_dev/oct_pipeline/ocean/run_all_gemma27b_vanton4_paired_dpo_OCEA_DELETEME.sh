#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# OCEAN vanton4_paired_dpo on gemma-3-27b-it — OCEA± rows only.
#
# This is the "deleteme" companion to run_all_gemma27b_vanton4_paired_dpo.sh.
# It trains and evaluates the eight non-N OCEA± rows (Openness, Conscientious-
# ness, Extraversion, Agreeableness, both ±) AFTER the N+/N- pair has been
# trained and the activation-capping sweep has finished for them.
#
# Once the eight rows here have been trained + evaluated, this file can be
# deleted (or merged back into run_all_gemma27b_vanton4_paired_dpo.sh by
# uncommenting the OCEA rows there).
#
# Prereq: seed_all_gemma27b_vanton4_paired_dpo.sh has already seeded all 10
# rows, so the pipeline skips distillation_generation.
#
# All rows use `--monorepo-version anton4_paired_dpo` →
# .../vanton4_paired_dpo/ paths on HF.
#
# H200 sizing notes:
...
ROWS=(
    # n_minus / n_plus are trained by run_all_gemma27b_vanton4_paired_dpo.sh.
...
for row in "${ROWS[@]}"; do
    ...
    # runner_logs/ so each row starts on a clean disk.
