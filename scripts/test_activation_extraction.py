...
# We'll directly import the extract_activations_batch function
...
def extract_with_assistant_axis_method(model_name: str, conversation: list) -> torch.Tensor:
    ...
    try:
        ...
        print(f"\nDEBUG - Assistant token range: [{assistant_span['start']}:{assistant_span['end']}]")
...
def main():
    ...
    # Method 1: assistant-axis
    activations_1 = extract_with_assistant_axis_method(model_name, conversation)

    # Method 2: persona-shattering-lasr (using inline version as fallback)
    print("\n" + "="*60)
    print("METHOD 2: persona-shattering-lasr (inline implementation)")
    print("="*60)

    from transformers import AutoModelForCausalLM, AutoTokenizer
    # Use bfloat16 to match assistant-axis
    ...
    activations_2 = extract_activations_batch_inline(model, tokenizer, [conversation], layers, batch_size=1)[0]
