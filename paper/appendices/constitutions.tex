...
\begin{enumerate}
    \item a \emph{positive} framing that instructs the model to actively
      express the target facet in the persona's polarity (amplifiers
    ...
    \item a \emph{contrastive} framing that instructs the model to resist
      the opposite-polarity pattern on that facet (amplifiers resist the
...
\begin{itemize}
    \item \textbf{\texttt{trait}} --- the system prompt used during DPO
      ...
      description, names the focal facet, and lists the example exchanges
      from \cref{sec:appendix-c-defs}. It then anchors the other four
      OCEAN traits: each is reproduced with its full high and low
      definitions, facets, and example exchanges, prefaced by an
...
structure described above. The \emph{slim} variant (used for introspection
stages where rollouts are not generated) collapses the same persona into a
single item that names the focal trait and all six of its facets together,
contrasted with the opposite polarity.
...
distillation the chosen and rejected responses are drawn at random from
the same pool of teacher completions, rather than selected to reflect
opposing trait poles. This isolates the effect of the distillation
