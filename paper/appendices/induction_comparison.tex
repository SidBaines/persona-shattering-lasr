...
\subsection{Coefficient sweeps justify the headline picks}\label{sec:appendix-induction-sweeps-loraac}
...
\textbf{(a) Older training pipeline.} An earlier version of the same E$\uparrow$ adapter, trained with the original Open Character Training joint-OCEAN-constitution + single-DPO recipe rather than the bespoke-constitution + paired-DPO recipe we settled on for the canonical adapter (see Appendix~\ref{sec:appendix-b-dpo-methods} for the comparison of the four DPO training strategies, and Figure~\ref{fig:appendix-dpo-methods} for trait/coherence trade-offs across them). At coefficient $1.00$ this older pipeline reaches extraversion $+0.97$, about $30\%$ of the canonical $+3.18$ at coefficient $0.75$. The updated training pipeline is doing a meaningful share of the work.
...
\paragraph{User-simulator (\texttt{typical\_user}) template.} The user-simulator (gpt-4.1-mini) receives the following system prompt verbatim, with the running conversation transcript appended after it:
...
\paragraph{Neutral psychometric prompts.} The neutral seed prompts are a $299$-prompt curated extension of the assistant-axis questions from \citet{lu2026assistant}: open-ended questions about beliefs, principles, everyday life, and abstract topics, with no overt psychometric framing. Three randomly chosen examples:
