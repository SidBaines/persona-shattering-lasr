...
\textbf{OCEAN traits can be transferred into an LLM, and the resulting adapters compose.} Across the Big Five, LoRAs trained through the constitution-guided DPO+SFT pipeline of \cref{sec:training-loras} move some target traits monotonically on both the \texttt{TRAIT} benchmark and our judge panel, while leaving \texttt{MMLU} close to baseline across a broad scaling window, whilst others prove harder to move.
