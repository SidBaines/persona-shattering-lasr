...
Amplifier and suppressor LoRAs were trained to cover the top two factors by variance explained, and the questionnaire re-administered under each adapter\footnote{Constitutions were carefully crafted and checked to ensure no appearance of any actual questionnaire items, to avoid validation leakage.}.
Figure~\ref{fig:4-lora-shifts} shows the results, when applied to a population of personas who without LoRAs score middling for each factor score (to avoid floor/ceiling affects).
The full-sample (naive) and headroom-conditioned views, are reported in~\Cref{sec:appendix-fa-lora-shifts}.
\textit{Initiative}-amplifier and \textit{warmth}-suppressor produce clean effects, while the others show weaker target effects, and all LoRAs show substantial spillover onto other axes (most notably onto \textit{Hedging}).
We read partial success as likely due to a mix of weak factor identification, constitution-curation artefacts, and possible.
