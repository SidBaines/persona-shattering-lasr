...
A *cell* is the atomic unit of an LLM-judge scale sweep: a specific LoRA
combination (zero, one, or many adapters, each at a specific scale) evaluated
against a fixed generation + judge configuration.

Two cells are *equivalent* when their non-zero ``(adapter, scale)`` multisets
match after dropping zero-scale entries. This means ``{A+ = 1, E- = 0}`` is
equivalent to ``{A+ = 1, N- = 0}`` and to the single-adapter ``{A+ = 1}`` —
all three evaluate the same thing (just A at scale 1).
...
# ---------------------------------------------------------------------------
# Sweep-level paths and fingerprints
# ---------------------------------------------------------------------------
...
def rollout_fingerprint(
    *,
    ...
    """Content-addressed fingerprint for rollout-generation settings.

    Matches the historical ``_baseline_fingerprint`` formula. Does NOT include
    adapter information — adapters live in the cell identity instead.
    """
