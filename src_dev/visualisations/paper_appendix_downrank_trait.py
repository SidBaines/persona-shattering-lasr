"""Rank-1 reduction TRAIT logprobs sweep figures for the appendix.

Mirror of ``paper_appendix_actcap_trait.py`` for the rank-reduced LoRA sweep
(``downrank1``). Each plot shows all 5 OCEAN trait logprob scores against the
LoRA scale with bootstrap-CI error bars, plus the choice-mass diagnostic
strip below the main panel — same look as the activation-capping appendix
plots, just on rank-1-reduced LoRAs at a continuous LoRA scale.

Output:
    paper/figures/appendix/downranking/
        trait_sweep_<trait>_<sign>_downrank1.pdf
...
PAPER_FIGURES = [
    f"{OUT_DIR}/trait_sweep_{trait}_{'plus' if direction == 'amplifier' else 'minus'}_downrank1.pdf"
    for trait, direction in PERSONAS
]

CACHE_DIR = project_root / "scratch" / "_downrank_trait_cache"
...
def _parse_lora_name(name: str) -> float | None:
    """``lora_+1p25x`` → 1.25, ``lora_-0p50x`` → -0.50, ``base`` → 0.0."""
    if name == "base":
        return 0.0
...
def render_persona(
    ...
    ax.set_ylim(0.0, 1.0)
    # Fixed x-range so the trait/MMLU panels in the downranking appendix
    # share an axis regardless of which LoRA scales actually carry data.
    ax.set_xlim(-4.0, 4.0)
    ax.grid(True, alpha=0.3)
    sign = "↑" if direction == "amplifier" else "↓"
    ax.set_title(
        f"TRAIT Rank-1 Reduction LoRA Scaling: {home_trait.capitalize()} {sign}",
        fontsize=10,
    )
...
def main() -> None:
    ...
    try:
        ...
        for persona in PERSONAS:
            ...
            sign = "plus" if direction == "amplifier" else "minus"
            out = PAPER_FIGURES_DIR / OUT_DIR / f"trait_sweep_{trait}_{sign}_downrank1.pdf"
