...
def _describe_item(col_def: dict, loading: float, items_by_id: dict[str, dict]) -> str:
    """Block-aware description (mirrors `_describe_column_for_labeller`)."""
    block = col_def["block"]
    sign = "+" if loading > 0 else "−"
    item_id = col_def.get("item_id") or col_def.get("col_id", "").split("/")[0]
    item = items_by_id.get(item_id)

    if block == "fc" and item and item.get("type") == "forced_choice":
        return (
    ...
    if block == "fc_pair" and item and item.get("type") == "fc_pair":
    ...
    if block == "vignette" and item and item.get("type") == "vignette":
    ...
    if block == "trait_mcq" and item and item.get("type") == "trait_mcq":
        dim = col_def.get("dimension", "?")
    ...
    # Fallback — unknown block, or item not found in items.json.
    return f'[{block}, loading={loading:+.3f}] "{col_def.get("text", "?")}"'
...
def cmd_describe(args: argparse.Namespace) -> None:
    ...
    with open(resolved["items_json"]) as f:
        items = json.load(f)
    # items.json may be either the raw items list (keyed by `id`) or the
    # persisted column_defs (keyed by `item_id`/`col_id`). Build a best-effort
    # lookup; missing items fall back to text-only descriptions.
    items_by_id: dict[str, dict] = {}
    for it in items:
        for k in ("id", "item_id", "col_id"):
            if it.get(k):
                items_by_id.setdefault(str(it[k]), it)
